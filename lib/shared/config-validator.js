...
/* ec0lint class-methods-use-this: "off" */
...
export default class ConfigValidator {
    ...
    validateEnvironment(environment, source, getAdditionalEnv = noop) {
        ...
        Object.keys(environment).forEach((id) => {
            const env =
                getAdditionalEnv(id) || BuiltInEnvironments.get(id) || null;
        ...
        Object.entries(globalsConfig).forEach(
            ([configuredGlobal, configuredValue]) => {
                try {
                    ...
                    throw new Error(
                        `ec0lint configuration of global '${configuredGlobal}' in ${source} is invalid:\n${err.message}`
                    );
                }
            }
        );
    ...
    validateProcessor(processorName, source, getProcessor) {
        if (processorName && !getProcessor(processorName)) {
            throw new Error(
                `ec0lint configuration of processor in '${source}' is invalid: '${processorName}' was not found.`
            );
        }
    }
    ...
    validateConfigSchema(config, source = null) {
        ...
        if (!validateSchema(config)) {
            throw new Error(
                `ec0lint configuration in ${source} is invalid:\n${this.formatErrors(
        ...
        if (Object.hasOwnProperty.call(config, "ecmaFeatures")) {
            emitDeprecationWarning(source, "EC0LINT_LEGACY_ECMAFEATURES");
        }
    }
    ...
    validateConfigArray(configArray) {
        const getPluginEnv = Map.prototype.get.bind(
            configArray.pluginEnvironments
        );
