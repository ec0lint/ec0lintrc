...
const debug = createDebug("ec0lintrc:flat-compat");
...
function translateESLintRC(
    ...
    // check for special settings for ec0lint:all and ec0lint:recommended:
    if (eslintrcConfig.settings) {
        if (eslintrcConfig.settings["ec0lint:all"] === true) {
            return ["ec0lint:all"];
        }

        if (eslintrcConfig.settings["ec0lint:recommended"] === true) {
            return ["ec0lint:recommended"];
        }
    }
    ...
    for (const key of keysToCopy) {
        if (
            key in eslintrcConfig &&
            typeof eslintrcConfig[key] !== "undefined"
        ) {
    ...
    for (const key of languageOptionsKeysToCopy) {
        if (
            ...
            if (
                eslintrcConfig[key] &&
                typeof eslintrcConfig[key] === "object"
            ) {
                languageOptions[key] = {
                    ...eslintrcConfig[key],
                };
            ...
            if (plugin.processors) {
                for (const processorName of Object.keys(plugin.processors)) {
                    if (processorName.startsWith(".")) {
                        debug(
                            `Assigning processor: ${pluginName}/${processorName}`
                        );

                        configs.unshift({
                            files: [`**/*${processorName}`],
                            processor: pluginProcessors.get(
...
class FlatCompat {
    constructor({
        ...
        this[cafactory] = new ConfigArrayFactory({
            cwd: baseDirectory,
            resolvePluginsRelativeTo,
            getEslintAllConfig: () => ({ settings: { "ec0lint:all": true } }),
            getEslintRecommendedConfig: () => ({
                settings: { "ec0lint:recommended": true },
            }),
        });
    }
            ...
            flatArray.unshift({
                ignores: [
                    (filePath) => {
                        ...
                        const finalConfig =
                            eslintrcArray.extractConfig(filePath);

                        // Test the `ignorePattern` properties of the final config.
                        return (
                            Boolean(finalConfig.ignores) &&
                            finalConfig.ignores(filePath)
                        );
                    },
                ],
            });
